...
class CameraDevice():
    ...
    def get_jpeg_frame_sync(self):
        ...
        frame = self.rotate(frame)
        self.base64_img(frame)
        cv2.imwrite('/var/data/frame.jpg', frame)
        # with open("/var/data/base64frame.txt", "w") as text_file:
...
async def feeder_logo(request):
    content = open(os.path.join(ROOT, 'client/bird-house.svg'), 'r').read()
    return web.Response(content_type='image/svg+xml', text=content)
...
async def gallery(request):
    ...
    cursorObj.execute('SELECT * FROM activity')
    rows = cursorObj.fetchall()
    data = []
    for row in rows:
        data.append({'id': row[0], 'name': row[1], 'label': row[2], 'typ': row[3] })
    return web.Response(content_type='application/json', body=json.dumps(data))

async def clean(request):
    
    cursorObj = con.cursor()
    cursorObj.execute('DELETE FROM activity')
    con.commit()
    data = {'success': True}
    for f in os.listdir('/var/media'):
        os.remove(os.path.join('/var/media', f))
...
async def post_activity(request):
    params = await request.json()
    id = int(round(time.time() * 1000))
    insert_activity(con,(id,params['name'],params['label'],params['typ']))
...
def create_table(con):
    cursorObj = con.cursor()
    cursorObj.execute('create table if not exists activity(id integer, name text, label text, typ text)')
    con.commit()

def insert_activity(con,entities):
    cursorObj = con.cursor()
    cursorObj.execute('INSERT INTO activity(id, name, label, typ) VALUES(?, ?, ?, ?)', entities)
    con.commit()
...
if __name__ == '__main__':
    ...
    ws = create_connection("ws://face:8080")
    ...
    app.router.add_get('/bird-house.svg',feeder_logo)
    ...
    app.add_routes([web.static('/content', '/var/media')])
